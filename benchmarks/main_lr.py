...
# disabling tqdm for hpc runs, TODO update to use an environment variable to disable
...
def get_data(aggregation_interval, num_timesteps, lookahead, run, features, data_dir, infile, task, base_path, outcome_var, force_binary_outcome=False):
    ...
    # Do we need this? the performance was the same without
    #elif "readmission" in outcome_var:
    #    print(f"Converting {outcome_var} from bools to ints")
    #    windows.loc[windows[outcome_var] == True, outcome_var] = 1
    #    windows.loc[windows[outcome_var] == False, outcome_var] = 0
...
def main(args):
    ...
    force_binary_outcome = args.force_binary_outcome
    infile = args.infile
    ...
    inputs, outcomes, train_idx, valid_idx, test_idx, pid_vids, input_timesteps = \
        get_data(aggregation_interval, num_timesteps, lookahead, run, features, data_dir, infile, task, base_path, outcome_var, force_binary_outcome)

    # fit on training data, we aren't using the validation set for this model, important that class_weight is set for this version of sklearn
    print("Fitting LogisticRegression from the training data set")
    clf = LogisticRegression(random_state=0, class_weight='balanced').fit(inputs[train_idx], outcomes[train_idx])
    ...
    unique_base_path = f'{task}'
    for a in include_in_path:
        unique_base_path += '_'+a+'_'+str(getattr(args, a))
    print(f"unique_base_path: {unique_base_path}")
    scores_path = f"{results_dir}/{unique_base_path}.json"

    print(f"Making sure results_dir {results_dir} exists")
